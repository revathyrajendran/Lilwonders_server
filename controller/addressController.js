...
exports.addAddressByUSERController=async(req,res)=>{
          ...
          const{customername,buildingname,locality,pincode,phonenumber,alternatenumber,addresstype,productcode, orderstatus}=req.body
           //req payload comes from jwtMiddleware.
             const usermail = req.payload
             try{
                  const newAddress = new addresses({
                    customername,buildingname,locality,pincode,phonenumber,alternatenumber,addresstype,productcode, orderstatus,usermail
                  })
...
exports.getAllOrdersForAdminController=async(req,res)=>{
  ...
  try{
    const allOrders = await addresses.find()
